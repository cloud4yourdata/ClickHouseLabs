...
CREATE MATERIALIZED VIEW IF NOT EXISTS nyc_taxi_stats_mv TO nyc_taxi_stats
AS SELECT
    year(toDate(pickup_datetime)) AS year,
    month(toDate(pickup_datetime)) AS month,
...
insert into nyc_taxi_key
    SELECT
    vendor_id,
    pickup_datetime + toIntervalYear(2) AS pickup_datetime_new,
    dropoff_datetime + toIntervalYear(2) AS dropoff_datetime_new,
    passenger_count,
    trip_distance,
    ratecode_id,
    pickup_location_id,
